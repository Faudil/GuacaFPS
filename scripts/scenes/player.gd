...
func init(player_info: Dictionary):
	_pseudo = player_info["pseudo"]
	_avatar_color = player_info["avatar_color"]
	var mat = preload("res://assets/materials/player_material.tres")
	mat.albedo_color = Color(_avatar_color)
...
func server_process(delta: float):
	...
	if input.firing and not weapon.is_firing():
		...
		fire()
		input.firing = false
	# First we rotate the camera once on the server
	rotate_camera(input.head_y_axis, input.camera_x_axis)
	# Now we iterate through all clients except the one who sent us the data
